...
const char* getStrokeText()
{
    ...
    VIZ_GameStateUpdateStrokes(balls[0].getStrokes(), balls[1].getStrokes());
...
const char* getLevelText(int side)
{
	...
	VIZ_GameStateUpdateLevel(tempLevel - 1, tempLevel);
...
void update(VIZMessage msg)
{
	...
	if (state == 1)
	{
		for (Ball& b : balls)
		{
			b.update(deltaTime, mouseDown, mousePressed, tiles, holes, chargeSfx, swingSfx, holeSfx, msg);
		}
...
void graphics()
{
	...
	BALLPos ball_pos_0;
	BALLPos ball_pos_1;
	ball_pos_0.x = balls[0].getPos().x;
	ball_pos_0.y = balls[0].getPos().y;
	ball_pos_1.x = balls[1].getPos().x;
	ball_pos_1.y = balls[1].getPos().y;

	//std::cout << "balls[0].getStrokes() = " << balls[0].getStrokes() << std::endl;
	//std::cout << "balls[1].getStrokes() = " << balls[1].getStrokes() << std::endl;

	VIZ_GameStateUpdateBallPos(ball_pos_0, ball_pos_1);
...
{
	const char *viz_instance_id = "test";
	...
	const char *viz_instance_id_1 = "test1";
	const char *viz_instance_id_2 = "test2";
	VIZ_MQInit(viz_instance_id_1, viz_instance_id_2);
