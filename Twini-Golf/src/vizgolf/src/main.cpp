...
RenderWindow window("Twini-Golf", window_width, window_height);
...
std::vector<Hole> holes = {Hole(Vector2f(0, 0), holeTexture)};
...
int state = 1; //1 = game, 2 = end screen
...
const char* getStrokeText()
{
    ...
    VIZ_GameStateUpdateStrokes(balls[0].getStrokes(), 0);
...
void update()
{
	...
	if (state == 1)
	{
		balls[0].update(deltaTime, mouseDown, mousePressed, tiles, holes, chargeSfx, swingSfx, holeSfx, 0);
...
void game()
{
	...
	if (msg.command[0] == '1') {
		VIZ_GameStateUpdateReward(0.0);

		//printf("msg.code: %d\n", msg.code);
		//printf("msg.command: %s\n", msg.command);
		//printf("animation_flag: %d\n", animation_flag);
		//printf("\n");

		ball_action = msg.command;
		animation_flag = true;

		while (animation_flag == true) {
			update();
			graphics();
		}
		...
		VIZ_MQSend(VIZ_MSG_CODE_COMMAND, std::string("action_end").c_str());
	}
	else if (msg.command[0] == '2') {
		printf("msg.code: %d\n", msg.code);
		printf("msg.command: %s\n", msg.command);
		printf("\n");

		ball_action = "3$+0.0000$+0.0000$";
	}
	else if (msg.command[0] == '3') {
		for (int i = 0; i < 500; i++ ) {
			update();
			graphics();
		}
...
int main(int argc, char* args[])
{
	...
	//	printf("args[i]: %s\n", args[i]);
	//}
	const char *viz_instance_id = args[2];
