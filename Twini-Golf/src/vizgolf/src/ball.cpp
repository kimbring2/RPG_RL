...
void Ball::update(double deltaTime, bool mouseDown, bool mousePressed, std::vector<Tile> tiles,std::vector<Hole> holes, Mix_Chunk* chargeSfx, 
                  Mix_Chunk* swingSfx, Mix_Chunk* holeSfx, VIZMessage msg)
{   
    if ( msg.command[0] == '1' )
    {
        char velocity_x_buf[5];
        for (int i = 0; i < 5; i++ ) {
           velocity_x_buf[i] = msg.command[3 + i];
        }

        char velocity_y_buf[5];
        for (int i = 0; i < 5; i++ ) {
           velocity_y_buf[i] = msg.command[11 + i];
        }
    ...
    {
        //printf("if (mousePressed && canMove)\n");
